...
serial_port = None  # Added to prevent undefined variable error
...
def calculate_servo_angle(offset_x, offset_y):
    angle_pan = offset_x / M_PAN
    angle_tilt = offset_y / M_TILT
    
    return angle_pan, angle_tilt
    
def send_and_wait_for_echo(angle_pan, angle_tilt):
    

    # 2) Build the ASCII message "(a,b)\r\n"
    expected = f"({angle_pan},{angle_tilt})"
    msg = expected + "\r\n"
    ...
    for i in range(len(detections)):
        ...
        # Draw box if confidence threshold is high enough
            ...
            # Basic example: count the number of objects in the image
                ...
                angle_pan, angle_tilt = calculate_servo_angle(offset_x, offset_y)
                print(f"Person detected. Offset (x, y): ({offset_x}, {offset_y})")
                send_and_wait_for_echo(angle_pan, angle_tilt)
